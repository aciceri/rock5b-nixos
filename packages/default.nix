{
  ...
  perSystem = {lib, system, self', pkgs, ...}: let
    pkgsCross = system: (import inputs.nixpkgs {localSystem = system; crossSystem = "aarch64-linux";});
    evalConfig = import "${inputs.nixpkgs}/nixos/lib/eval-config.nix";
    buildConfig = hostSystem: config: evalConfig {
      system = hostSystem;
      modules = [
        config
